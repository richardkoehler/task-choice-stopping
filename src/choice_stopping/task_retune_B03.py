...
win = visual.Window(
    size=(1920, 1200), fullscr=False, screen=2,
...
skip_with_space=visual.TextStim(win=win, text='Leertaste drücken um zu starten', pos=(0.0, -0.25), height=0.03, color='white')
...
n_trials = 180
objects = [visual.Rect(win=win, width=0.15, height=0.15, fillColor="aqua"),
           visual.Rect(win=win, width=0.15, height=0.15, fillColor="yellow"),
           visual.Rect(win=win, width=0.15, height=0.15, fillColor="fuchsia"),
           visual.TextStim(win=win, text="Keine \nBewegung", color='lime', height=0.05, bold=True)]

# Fixation cross
cross_pos = -0.4
cross = visual.ShapeStim(win=win, lineColor='gray',
                         vertices=((0, cross_pos-0.04), (0, cross_pos+0.04), (0, cross_pos), (-0.04, cross_pos), (0.04, cross_pos)),
                         closeShape=False)
...
for trial in range(n_trials):
    ...
    while trial_time.getTime() < 2.5:
        ...
        dataFile.write('%i,%i, %i, %i, %.5f, %.5f, %.5f\n' % (trial, 0, condition, stopped, trial_time.getTime(), mouse_pos[0], mouse_pos[1]))
        win.flip()
    ...
    while trial_time.getTime() < 2.3:
        ...
        # Save
        dataFile.write('%i, %i, %i, %i, %.5f, %.5f, %.5f\n' % (trial, 1, condition, stopped, trial_time.getTime(), mouse_pos[0], mouse_pos[1]))
